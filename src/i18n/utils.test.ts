...
describe("buildPrefix", () => {
  ...
  describe('for "/blog" as Base Url', () => {
    it('should return "/blog" for default locale "es"', () => {
      const prefix = buildPrefix(DEFAULT_LOCALE, DEFAULT_LOCALE, "/blog");
      expect(prefix).toBe("/blog");
    });

    it('should return "/blog/ja" for non-default locale "ja"', () => {
      const prefix = buildPrefix("ja" as LocaleKey, DEFAULT_LOCALE, "/blog");
      expect(prefix).toBe("/blog/ja");
    });
  });
});
